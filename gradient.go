...
// Gradient defines a linear colour gradient between two hex colours.
// Start and End are lipgloss.Color values in "#rrggbb" form.
type Gradient struct {
	Start lipgloss.Color
	End   lipgloss.Color
}

// RenderAt returns a hex colour for the given interpolation point t in [0,1].
...
// RenderText applies the gradient across each rune of s, returning an ANSI-styled string.
// Each character is coloured at an evenly-spaced step along the gradient.
...
// RenderGradient is a package-level convenience wrapper for Gradient.RenderText.
func RenderGradient(text string, g Gradient) string {
	return g.RenderText(text)
}

// parseHex parses a "#rrggbb" colour string into r, g, b in [0,255].
